...
2. Download the latest release of the mod. Copy the folders inside into the "LobotomyCorp_Data" directory in the game's files, replacing files when asked. If you wish to compile the mod yourself, then assemble the compiled files with the same file structure as the release.
3. Optional - install the following quality of life mods (others may work, but I am only testing with the following):
...
4. Open the Connector folder's config file. Set the "NeuroURI" key's value to whatever websocket server you're using. Alternatively, set the "NEURO_SDK_WS_URL" environment variable instead.
5. The game-to-server requests and server-to-game requests run on http://127.0.0.1:8080 and http://127.0.0.1:8081 respectively by default. Either ensure nothing else is running on those servers, or, change the "GameToServerURI" and "ServerToGameURI" keys in both the Mod and Connector config files. These must be identical in both config files, and both be http or https URIs.
6. Launch the game through the Lobotomy Mod Manager. This will automatically open the secondary program which will connect to whatever Neuro SDK program you're using. This secondary program runs without a window; it should automatically close along with the game, otherwise, you can kill it with Task Manager.
